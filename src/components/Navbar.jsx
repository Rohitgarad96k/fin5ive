...
const Navbar = () => {
  ...
  // Exact links matching your design screenshot
  const serviceLinks = [
    ...
    { name: 'NRI Services', path: '/services/nri' },
        ...
        <div className="flex justify-between items-center h-full">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/" className="flex items-center gap-3">
              ...
              {/* Optional: You can delete this div if your logo image already contains the "FIN5IVE Ambitions Realised" text */}
              <div className="flex flex-col">
                <span className="text-2xl md:text-3xl font-extrabold text-finBlue tracking-tight">FIN<span className="text-finOrange">5</span>IVE</span>
          ...
          <div className="hidden md:flex items-center h-full space-x-8">
            <Link
              to="/"
              className={`font-medium text-[15px] transition-colors hover:text-[#FF6600] ${location.pathname === '/' ? 'text-[#FF6600]' : 'text-[#003366]'}`}
            >
              Home
            </Link>
            ...
            <div className="relative group h-full flex items-center">
              {/* "Our Services" is now a clickable link to /services */}
              <Link
                to="/services"
                className={`flex items-center gap-1 font-medium text-[15px] h-full transition-colors hover:text-[#FF6600] ${location.pathname.includes('/services') ? 'text-[#FF6600]' : 'text-[#003366]'}`}
              >
                Our Services <ChevronDown className="w-4 h-4 ml-0.5" />
              </Link>

              {/* Dropdown Card - Matches your screenshot exactly */}
              <div className="absolute top-[75px] left-0 bg-white border border-gray-100 shadow-xl py-2 w-64 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform">
                {serviceLinks.map((service, index) => (
                  <Link
                    key={index}
                    to={service.path}
                    className="block px-6 py-2.5 text-[14px] text-[#003366] hover:bg-slate-50 hover:text-[#FF6600] transition-colors"
                  >
                    {service.name}
                  </Link>
                ))}
            ...
            <Link
              to="/about"
              className={`font-medium text-[15px] transition-colors hover:text-[#FF6600] ${location.pathname === '/about' ? 'text-[#FF6600]' : 'text-[#003366]'}`}
            >
              About Us
            </Link>

            {/* Client Portal Button */}
            <Link to="/portal" className="bg-[#003366] hover:bg-[#002244] text-white px-5 py-2.5 rounded font-medium text-[15px] transition-colors flex items-center shadow-sm ml-4">
            ...
            <div className={`bg-slate-50 overflow-hidden transition-all duration-300 ${isServicesOpen ? 'max-h-[500px]' : 'max-h-0'}`}>
              {serviceLinks.map((service, index) => (
                <Link
                  key={index}
                  to={service.path}
                  className="block px-8 py-3 text-sm text-gray-600 font-medium hover:text-[#FF6600] border-b border-gray-100/50"
                >
                  {service.name}
                </Link>
              ))}
